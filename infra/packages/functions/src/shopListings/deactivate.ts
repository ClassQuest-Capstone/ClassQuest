 ...
 *
 * Set is_active = false on a ShopListing.
...
export const handler = async (event: APIGatewayProxyEventV2) => {
    try {
        ...
        const updated = await setListingActiveStatus(current.PK, current.SK, current.class_id, false, now);
        ...
        console.error("Error deactivating shop listing:", error);
        return {
            statusCode: 500,
            headers: { "content-type": "application/json" },
