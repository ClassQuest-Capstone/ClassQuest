...
export function getQuestTemplatesByOwner(teacher_id: string) {
    return api<{ items: QuestTemplate[] }>(`/quest-templates/owner/${encodeURIComponent(teacher_id)}`);
}

export function getPublicQuestTemplates(subject?: string, grade?: number, difficulty?: Difficulty) {
    const params = new URLSearchParams();
    if (subject) params.append("subject", subject);
    if (grade) params.append("grade", grade.toString());
    if (difficulty) params.append("difficulty", difficulty);
    
    return api<{ items: QuestTemplate[] }>(`/quest-templates/public?${params.toString()}`);
}

export function updateQuestTemplate(quest_template_id: string, data: Partial<CreateQuestTemplateInput>) {
    return api<QuestTemplate>(`/quest-templates/${encodeURIComponent(quest_template_id)}`, {
        method: "PUT",
        body: JSON.stringify(data),
    });
...
export function deleteQuestTemplate(quest_template_id: string) {
    return api<void>(`/quest-templates/${encodeURIComponent(quest_template_id)}`, {
        method: "DELETE",
    });
}
